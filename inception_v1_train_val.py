...
input_images = tf.placeholder(dtype=tf.float32,
                              shape=[None, resize_height, resize_width, depths], name='input')
# 定义input_labels为labels数据
input_labels = tf.placeholder(dtype=tf.int32,
                              shape=[None, labels_nums], name='label')
# 定义dropout的概率
...
def step_train(train_op,loss,accuracy,
    ...
    with tf.Session() as sess:
        ...
        for i in range(max_steps + 1):
            ...
            #    acc_summary = scalar_summary('accuracy', train_acc)
            ...
            if i % val_log_step == 0:
                ...
                with open(val_loss_txt, "a+") as  f3:
                    f3.write(str(i))
                    f3.write("\t")
                    f3.write(str(mean_loss))
                    f3.write("\n")
                f3.close()

                with open(val_acc_txt, "a+") as  f4:
                    f4.write(str(i))
                    f4.write("\t")
                    f4.write(str(mean_acc))
                    f4.write("\n")
                f4.close()
        ...
        #summary, _ = sess.run([merged, 100], feed_dict={x: x_batch, y: y_batch})
        #counter += 1
...
if __name__ == '__main__':
    ...
    train_loss_txt = 'dataset/train_loss.txt'
    train_acc_txt = 'dataset/train_acc.txt'
    val_loss_txt = 'dataset/val_loss.txt'
    val_acc_txt = 'dataset/val_acc.txt'
    ...
    snapshot_prefix='models8/model.ckpt'
